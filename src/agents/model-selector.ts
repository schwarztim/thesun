...
export function selectModel(context: ModelSelectionContext): ClaudeModel {
  const { phase, taskType, complexity, iterationCount, isSecuritySensitive } =
    context;

  // Security-sensitive tasks always use Opus
  if (isSecuritySensitive) {
    logger.debug("Model selection: Opus (security sensitive)", {
      taskType,
      phase,
    });
    return "opus";
  ...
  if (PHASE_OVERRIDES[phase]) {
    ...
    logger.debug(`Model selection: ${model} (phase override)`, {
      phase,
      taskType,
    });
    return model;
  ...
  if (iterationCount && iterationCount > 1) {
    ...
    logger.debug(`Model selection: ${model} (iteration ${iterationCount})`, {
      taskType,
      phase,
    });
    return model;
  ...
  if (complexity === "high") {
    logger.debug("Model selection: Opus (high complexity)", {
      taskType,
      phase,
    });
    return "opus";
 ...
 */
export function getModelConfig(model: ClaudeModel): {
  model: string;
  maxTokens?: number;
} {
  switch (model) {
 ...
 */
export function selectModelForPhase(
  phase: BuildPhase,
  iterationCount = 0,
): ClaudeModel {
